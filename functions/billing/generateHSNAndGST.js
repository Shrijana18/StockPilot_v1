...
module.exports = onCall(async (request) => {
  try {
    const { productName, category, description } = request.data || {};
    if (!productName && !category) {
      throw new Error("Missing productName or category");
    }
...
`;
    ...
    if (!parsed) {
      return { success: false, message: "No valid JSON returned", raw: rawText };
    }
