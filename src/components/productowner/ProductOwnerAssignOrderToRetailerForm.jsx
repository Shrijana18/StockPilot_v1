...
const ProductOwnerAssignOrderToRetailerForm = ({ retailer, onClose, onSuccess }) => {
  ...
  // Handle opening inventory modal for specific item index
  ...
  const handleSubmit = async (e) => {
    ...
    if (!retailer?.retailerId) {
      toast.error('Retailer information missing');
      return;
    }
    ...
    try {
      // Fetch Product Owner's business information
      ...
      // Fetch Retailer's business information
      const retailerBusinessRef = doc(db, 'businesses', retailer.retailerId);
      const retailerBusinessSnap = await getDoc(retailerBusinessRef);
      const retailerBusinessData = retailerBusinessSnap.exists() ? retailerBusinessSnap.data() : {};
      
      const itemsSubTotal = getItemsSubTotal();
      const orderId = doc(collection(db, `businesses/${retailer.retailerId}/productOwnerOrders`)).id;
      ...
      const orderPayload = {
        ...
        retailerCity: retailer.retailerCity || retailer.city || retailerBusinessData.city || '',
        retailerState: retailer.retailerState || retailer.state || retailerBusinessData.state || '',
        retailerAddress: retailer.retailerAddress || retailer.address || retailerBusinessData.address || '',
        ...
        statusHistory: [{
          ...
          notes: 'Order assigned to retailer by product owner',
        }],
      };

      // Save to retailer's productOwnerOrders
      await setDoc(doc(db, `businesses/${retailer.retailerId}/productOwnerOrders/${orderId}`), orderPayload);

      // Mirror to product owner's assignedOrdersToRetailers
      await setDoc(doc(db, `businesses/${productOwnerId}/assignedOrdersToRetailers/${orderId}`), {
        ...orderPayload,
      });

      toast.success('Order assigned to retailer successfully!');
    ...
    <>
      ...
      >
        ...
        <div className="sticky top-0 bg-slate-900/95 backdrop-blur-md border-b border-white/10 p-6 flex items-center justify-between z-10">
          <div>
            <h2 className="text-2xl font-bold text-white">Assign Order to {retailer?.businessName || retailer?.retailerName || 'Retailer'}</h2>
...
export default ProductOwnerAssignOrderToRetailerForm;
