...
export default function TableManager({ open, onClose, onTableCreated, editingTable = null, existingTables = [] }) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => { onClose(); setShowDeleteConfirm(false); setError(""); }}>
          <motion.div
          ...
          >
            ...
            <div className="mb-5 flex items-start justify-between">
              <div>
                <h2 className="text-lg font-bold text-white">{editingTable ? "Edit Table" : "Add New Table"}</h2>
                <p className="text-xs text-white/50 mt-0.5">{editingTable ? `Table ${editingTable.number} · ${editingTable.zone || "main"}` : "Quick setup for your restaurant"}</p>
              </div>
              {editingTable && (
                <button
                  type="button"
            ...
            {showDeleteConfirm && (
              <div className="mb-4 p-3 rounded-xl border border-red-500/30 bg-red-500/10">
                <div className="text-sm font-semibold text-red-300 mb-1">Delete this table?</div>
                <div className="text-xs text-red-400/70 mb-3">This cannot be undone. Active orders won't be deleted.</div>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => setShowDeleteConfirm(false)}
                    className="flex-1 py-1.5 rounded-lg border border-white/10 bg-white/5 text-white text-xs hover:bg-white/10 transition"
                    disabled={loading}
                  >Cancel</button>
                  <button
                    type="button"
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              {!editingTable ? (
                <div>
                  <label className="block text-xs font-semibold text-white/70 mb-1.5">Table Number *</label>
                  <input
                    type="number"
                    value={tableNumber}
                    ...
                    required
                    autoFocus
                    className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-white placeholder:text-white/30 focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/20 focus:outline-none transition"
                  />
                </div>
              ) : (
                <div>
                  <label className="block text-xs font-semibold text-white/70 mb-1.5">Table Number</label>
                  <div className="w-full rounded-lg border border-white/[0.07] bg-white/[0.03] px-4 py-2.5 text-white/50 text-sm">
                    {editingTable.number}
                  </div>
                </div>
              )}
              ...
              {/* Table Name */}
              <div>
                <label className="block text-xs font-semibold text-white/70 mb-1.5">Table Name <span className="text-white/30 font-normal">(optional)</span></label>
                <input
                  type="text"
                  value={tableName}
              ...
              <div className="grid grid-cols-2 gap-3">
                {/* Capacity */}
                <div>
                  <label className="block text-xs font-semibold text-white/70 mb-1.5">Capacity</label>
                  <input
                    type="number"
                    value={capacity}
                ...
                {/* Zone — text input with datalist */}
                <div>
                  <label className="block text-xs font-semibold text-white/70 mb-1.5">Zone</label>
                  <input
                    type="text"
              ...
              <div className="flex gap-3 pt-1">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-white text-sm hover:bg-white/10 transition"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-2.5 text-sm font-semibold text-white shadow-lg hover:shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed transition"
                >
                  {loading
